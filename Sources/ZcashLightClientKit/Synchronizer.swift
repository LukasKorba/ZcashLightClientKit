...
/// Mode of the Synchronizer's initialization for the wallet. The SDK now derives the
/// initialization flow from account existence and birthday presence.
...
extension Synchronizer {
    /// Compatibility shim for the pre-slipstream initialization API. The SDK derives the
    /// wallet-initialization flow itself; `walletMode` only decides whether the birthday
    /// is forwarded (`.newWallet` maps to a nil birthday, everything else passes it through).
    @available(*, deprecated, message: "Use prepare(with:walletBirthday:name:keySource:) — pass a nil birthday for a brand-new wallet.")
    public func prepare(
        with seed: [UInt8]?,
        walletBirthday: BlockHeight,
