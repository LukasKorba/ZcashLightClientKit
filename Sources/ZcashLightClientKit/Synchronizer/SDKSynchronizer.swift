...
//        }
    ...
    public func refreshExchangeRateUSD() async {
        ...
        do {
            if tor == nil {
                ...
                if let torService = initializer.container.resolve(LightWalletService.self) as? LightWalletGRPCServiceOverTor {
                    ...
                    try await tor?.initializeNewRuntime()
                }
            }
            // broadcast new value in case of success
            let exchangeRateUSD = try await tor?.getExchangeRateUSD()
            exchangeRateUSDSubject.send(exchangeRateUSD)
        } catch {
            // broadcast cached value but update the state
