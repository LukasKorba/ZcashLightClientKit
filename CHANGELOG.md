...
- `WalletInitMode` and the `prepare(with:walletBirthday:for:name:keySource:)` overload are
  deprecated (source-compatible shims remain on the async, closure and Combine surfaces).
  The primary API is `prepare(with:walletBirthday:name:keySource:)` — the SDK derives the
  init flow, and a `nil` birthday means a brand-new wallet. See `MIGRATING.md`.

## Fixed
- A freshly submitted transaction now surfaces in `eventStream` (`foundTransactions`)
  immediately after a successful broadcast instead of waiting for the next sync round.
